...
const InvoicesAddPage = () => {
  ...
  const [totalQuantityGlobal, setTotalQuantityGlobal] = useState(0); // GLOBAL TOTAL QUANTITY

  // Update the fetchCustomerDetails function to return a promise and properly set state
  const fetchCustomerDetails = async (customerId) => {
    ...
    try {
      ...
      // Calculate total quantity across all challans
      let globalTotalQuantity = 0;

      // Extract products and calculate total quantity
      ...
      // Set the global total quantity
      setTotalQuantityGlobal(globalTotalQuantity);
      console.log("GLOBAL TOTAL QUANTITY:", globalTotalQuantity);

      const customerProductsList = Array.from(productsMap.values());
      setCustomerProducts(customerProductsList);

      // AUTO-SELECT ALL PRODUCTS - THIS IS THE KEY FIX
      const productIds = customerProductsList.map(product => product.product_id);
      console.log("Products to auto-select:", productIds);
      console.log("Products list:", customerProductsList);
      ...
      // Return the product IDs for use in handleCustomerSelect
      return productIds;
  ...
  // You can use totalQuantityGlobal anywhere in your component
  // For example, display it in your JSX:
  {/* <Typography>Total Quantity: {totalQuantityGlobal}</Typography> */ }
  ...
  const handleCustomerSelect = useCallback(
    async (dispatchOrderId) => {
      ...
      if (customerId) {
        // Wait for fetchCustomerDetails to complete and get the product IDs
        autoSelectedProductIds = await fetchCustomerDetails(customerId);
        console.log("Auto-selected product IDs from fetch:", autoSelectedProductIds);
      }
      ...
      if (selectedOrder.type === "dispatch_order") {
        ...
        // Use the product IDs from the order items
        const productIds = orderItems.map((item) => item.product_id);
        console.log("Order product IDs:", productIds);

        // If autoSelectedProductIds is empty, use productIds from order
        const finalSelectedProductIds = autoSelectedProductIds.length > 0
          ? autoSelectedProductIds
          : productIds;

        console.log("Final selected product IDs:", finalSelectedProductIds);

        // Set selected product IDs
        setSelectedProductIds(finalSelectedProductIds);
        ...
        const selectedProducts = products.filter((product) =>
          finalSelectedProductIds.includes(product.id)
        );
    ...
    [orders, products, taxTypes, formData, calculateRentalPrice, fetchCustomerDetails]
  );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submissionData = {
        ...
        // In the handleSubmit function, update the items mapping for direct invoices
        items: formData.items.map((item) => {
          ...
          // Get the selected order to access its items
          const selectedOrder = orders.find(
            (order) => order.id === parseInt(selectedOrderId)
          );

          if (selectedOrder?.type === "direct_invoice") {
            // Find the corresponding item in the selected order to get device_ids
            ...
            return {
              ...
              device_ids: deviceIds, // Use the actual device IDs from the order
              new_device_ids: [],
              returned_device_ids: [],
              rental_duration: formData.rental_duration || "0",
              rental_duration_days: formData.rental_duration_days || 0,
                            ...
                            onChange={() => {
                              if (
                                ...
                                customerProducts.forEach((product) => {
                                  newQuantities[product.product_id] =
                                    product.total_quantity;
                                });
                                setQuantities(newQuantities);
                                setSelectedProductIds(
                                  customerProducts.map((product) => product.product_id)
                                );
                              }
                            }}
                          />
                    ...
                    <TableBody>
                      {customerProducts
                        .filter((customerProduct) => {
                          ...
                          return (
                            customerProduct.product_name?.toLowerCase().includes(searchTermLower) ||
                            customerProduct.specifications?.model?.toLowerCase().includes(searchTermLower) ||
                            customerProduct.specifications?.processor?.toLowerCase().includes(searchTermLower) ||
                            customerProduct.specifications?.ram?.toLowerCase().includes(searchTermLower) ||
                            customerProduct.specifications?.storage?.toLowerCase().includes(searchTermLower)
                          );
                        })
                        .map((customerProduct) => (
                          <TableRow key={customerProduct.product_id}>
                            <TableCell padding="checkbox">
                              <Checkbox
                                ...
                                onChange={() =>
                                  handleProductSelection(customerProduct.product_id)
                                }
                                disabled
                              />
                            </TableCell>
                            <TableCell>{customerProduct.product_name}</TableCell>
                            <TableCell>
                              {[
                                customerProduct.specifications?.model,
                                customerProduct.specifications?.processor,
                                customerProduct.specifications?.ram,
                                customerProduct.specifications?.storage,
                                customerProduct.specifications?.graphics,
                              ]
                                .filter(Boolean)
                                .join(" | ")}
                            </TableCell>
                            <TableCell>
                              <Box display="flex" alignItems="center">
                                <IconButton
                                  size="small"
                                  onClick={() => decrementQty(customerProduct.product_id)}
                                  disabled
                                >
                                ...
                                <TextField
                                  ...
                                  size="small"
                                  value={
                                    customerProduct.total_quantity
                                  }
                                  onChange={(e) =>
                                    handleQtyChange(customerProduct.product_id, e.target.value)
                                  }
                                  disabled
                                  inputProps={{
                                    min: 0,
                                ...
                                <IconButton
                                  size="small"
                                  onClick={() => incrementQty(customerProduct.product_id)}
                                  disabled
                                >
                                  <Add fontSize="small" />
                                </IconButton>
