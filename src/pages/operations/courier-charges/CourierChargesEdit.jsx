  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch courier charge details
        const courierResponse = await axios.get(
          `${API_URL}/courier-charges/${id}`,
          {
            headers: { "Authorization": `Bearer ${userToken}` },
          }
        );
        ...
        if (courierResponse.status === 200) {
          const data = courierResponse.data;
          setFormData({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (response.status === 200) {
        // 4. Success
        showSnackbar("Courier Charges updated successfully", "success");
        setTimeout(() => {
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        <h1 style={titleStyle}>Edit Courier</h1>
      </div>
      ...
      <div style={buttonContainerStyle}>
        <button
          ...
          onClick={() => navigate("/dashboard/operations/courier-charges")}
        >
          Cancel
        </button>
        <button
...
// Modern Styles (same as in CourierChargesAdd)
...
export default CourierChargesEdit;
